...
def load_best_reference(metadata_path=None):
    """
    Load the best reference selected by the reference selector
    
    Args:
        metadata_path: Path to reference metadata JSON file (optional, uses default if None)
    ...
    if metadata_path is None:
        ...
        metadata_path = Path(__file__).parent / "stain_reference_metadata.json"
    else:
...
def complete_preprocessing_pipeline(image, normalizer, apply_zscore=True, zscore_mean=200.99, zscore_std=25.26, percentile_low=1.0, percentile_high=99.0):
    """
    ...
    Args:
        ...
        apply_zscore: Whether to apply z-score normalization after stain normalization
...
def main():
    ...
    try:
        ...
        if Path(example_path).exists():
            ...
            # Create comparison
            comparison_path = "stain_normalization_comparison.png"
