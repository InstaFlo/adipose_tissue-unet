#!/usr/bin/env python3
"""
Publication-Quality Full Evaluation Script for Adipose U-Net
Implements rigorous methodological standards for peer-reviewed publication
...
def set_deterministic_seeds(seed: int = None):
    ...
    # Load seed from seed.csv if not provided
    if seed is None:
        from src.utils.seed_utils import get_project_seed
        seed = get_project_seed()
...
def run_publication_evaluation(val_data_root: str, weights_path: str, output_dir: str,
                             ...
                             use_tta: bool = False, tta_mode: str = 'basic') -> ComprehensiveMetrics:
    """
    ...
    for i, (img_path, mask_path) in enumerate(paired_files):
        ...
        # Run prediction using training statistics with optional TTA (no data leakage!)
        pred_mask, timing_info = model.predict(image, train_mean, train_std, use_tta=use_tta, tta_mode=tta_mode)
    ...
    if optimize_threshold:
        ...
        # Use slide-level optimization to match reporting methodology
